...
def atualizar_senha()
    atualizar = tk.Toplevel(janela)
    atualizar.title("Atualizar senha")
    atualizar.geometry("400x300")

    frm_atualizarr = ttk.Frame(atualizar, padding=10)
    frm_atualizar.pack(expand=True)

    ttk.Label(frm_atualizar, text="Atualizar senha", font=("Arial", 16)).pack(pady=10)
...
def abrir_tela_inicial(usuario_logado):
    ...
    total_carrinho = tk.DoubleVar(value = 0.0)

def catalogo():

    frm_catalogo = ttk.Frame(frm)
    frm_catalogo.pack(pady=20)

    jogos = [
    ("The Witcher 3", 129.90, "imagens/the witcher3.png"),
    ("Red Dead Redemption 2", 299.90, "imagens/red dead.png"),
    ("DOOM ETERNAL", 149.00, "imagens/DOOM.png"),
    ("Minecraft", 79.90, "imagens/mine.png")
    ]

    for nome,preco in jogos:
        frame_jogo = ttk.Frame(frm_catalogo)
        frame_jogo.pack(fill="x", pady=5)
...
tk.Button(frm, text="Atualizar senha",command=atualizar_senha, bg="Blue", fg="white", activebackground="Dark Blue", activeforeground="Black").pack(pady=9)
